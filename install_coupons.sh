...
# ==================================================================================
# 🎄 Jennifer's Upgraded Christmas Coupon Server (v3) 🎄
# ==================================================================================
# Features:
# - Random Non-Sequential IDs
# - Subtle Christmas Styling
# - QR Code Printing (Double-Sided Business Card Layout 2x5)
# - Systemd Service
# ==================================================================================
...
new_items = [
    ...
    {"name": "Back Scratching (5m)", "category": "Relaxation"},
    {"name": "Foot Rub (15m)", "category": "Relaxation"},
    {"name": "Foot Rub (15m)", "category": "Relaxation"},
    {"name": "Back Massage (30m)", "category": "Relaxation"},
    {"name": "Back Scratching (10m)", "category": "Relaxation"},
    {"name": "Spa Day (Baby Watch)", "category": "Relaxation"},
    {"name": "Sleep In", "category": "Relaxation"},
    {"name": "24hr Mom Break", "category": "Relaxation"},
    {"name": "Tech Free Hour", "category": "Quality Time"},
    {"name": "Watch Movie (No Work)", "category": "Quality Time"},
    {"name": "Trip to Lake", "category": "Adventure"},
    {"name": "Go on a Hike", "category": "Adventure"},
    {"name": "Do Dishes (All Day)", "category": "Chores"},
    {"name": "Vacuum & Mop", "category": "Chores"},
    {"name": "Do Laundry (All Day)", "category": "Chores"},
    {"name": "Grocery Shopping", "category": "Chores"},
    {"name": "Organize A Room", "category": "Chores"},
    {"name": "Diaper Duty (All Day)", "category": "Chores"},
    {"name": "Pay For Gas (Tank)", "category": "Gifts"},
    {"name": "Personal Chauffeur", "category": "Service"},
    {"name": "One 'Yes' Day", "category": "Special"},
    {"name": "Wildcard #1", "category": "Special"},
    {"name": "Wildcard #2", "category": "Special"},
    {"name": "Breakfast in Bed", "category": "Food"},
    {"name": "Dinner Choice", "category": "Food"},
    {"name": "Ice Cream Date", "category": "Food"},
    {"name": "Game Night", "category": "Quality Time"},
    {"name": "Walk in the Park", "category": "Adventure"},
    {"name": "Car Wash", "category": "Chores"}
]
...
for item in new_items:
    while True:
        ...
        if new_id not in used_ids:
            used_ids.add(new_id)
            db["definitions"][new_id] = item["name"]
            db["categories"][new_id] = item["category"]
            break
...
class CouponHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        # Rewrite root to index.html (User View)
...
# 2.3.5 USER FRONTEND (index.html) - 3D Carousel
cat <<EOF > index.html
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print {
        ...
        .page { margin: 0; border: none; box-shadow: none; margin-bottom: 0; }
        .coupon, .coupon-back { border: 1px solid #eee; } /* Lighter border for print */
    }

    /* Screen visual separation */
    .page {
        background: white;
        margin-bottom: 40px;
        box-shadow: 0 5px 15px rgba(0,0,0,0.1);
...
<div class="screen-only">
    ...
    <p style="margin-bottom:15px">
        ...
        1. Click on text to edit description/title if needed.<br>
...
<script>
    async function load() {
        ...
        for (let i = 0; i < items.length; i += chunkSize) {
            ...
            pageFront.innerHTML = \`<div class="screen-only page-label">Page \${pageNum} (FRONT)</div>\`;
            ...
            chunk.forEach(([code, name]) => {
                ...
                div.innerHTML = \`
                    ...
                    <div class="coupon-info">
                        <div class="coupon-title" contenteditable="true">\${name}</div>
            ...
            pageBack.innerHTML = \`<div class="screen-only page-label">Page \${pageNum} (BACK)</div>\`;

            // Iterate by rows (pairs of 2) for mirroring
            for (let r = 0; r < 5; r++) {
    ...
    function createBackCard([code, name]) {
        ...
        div.innerHTML = \`
            ...
            <div class="back-title" contenteditable="true">\${name}</div>
            <div class="back-desc" contenteditable="true">Valid for one use. Merry Christmas!</div>
